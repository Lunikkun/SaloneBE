var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
import { eq } from 'drizzle-orm';
import { db } from './../../dbConnection.js';
import { saloonServices } from "./schema.js";
export function insertService(service) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield db.insert(saloonServices).values(service).returning();
    });
}
export function selectService(id) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield db.select()
            .from(saloonServices)
            .where(eq(saloonServices.id, id));
    });
}
export function selectAllServices() {
    return __awaiter(this, void 0, void 0, function* () {
        return yield db.select()
            .from(saloonServices);
    });
}
export function deleteService(id) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield db.delete(saloonServices)
            .where(eq(saloonServices.id, id))
            .returning();
    });
}
export function updateService(id, { nome, prezzo, descrizione }) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield db.update(saloonServices)
            .set({ nome: nome, prezzo: prezzo, descrizione: descrizione })
            .returning();
    });
}
